import YTDlpWrapType from "yt-dlp-wrap";

/**
 * @type {YTDlpWrapType}
 */
var ytDlpWrap = "./yt-dlp";
...
import { createWriteStream, unlinkSync, createReadStream, statSync, readdir, stat, existsSync, mkdirSync } from "fs";
import { get } from "https";
import { SingleBar, Presets } from "cli-progress";
import { parser } from "html-metadata-parser";
import ytdl, { validateURL } from "@distube/ytdl-core";
import fluentffmpeg, { ffprobe } from "fluent-ffmpeg";
import ytpl, { validateID, continueReq } from "ytpl";
...
const
  YT_COOKIE = "CONSENT=PENDING+108; APISID=8MoRhdzG5Q9kGpw5/AJj3Kpt8RtQ6Gp-B6; SAPISID=f2pBhOjXgIObITwf/A4PXz4_8t6WwXwmKY; __Secure-1PAPISID=f2pBhOjXgIObITwf/A4PXz4_8t6WwXwmKY; __Secure-3PAPISID=f2pBhOjXgIObITwf/A4PXz4_8t6WwXwmKY;",
  music_temp_dir = "music_temp/",
  authed_user_id = [393140530273452034],
...
class discord_music {
  ...
  async join_channel(args) {
    ...
    if (this.last_at_vc_channel) {
      var sameGuildConnection = this.connections.filter((e, ind, arr) => {
        ...
        if (
          e.joinConfig.guildId == this.last_at_vc_channel.guildId &&
      ...
      if (
        !this.connections.find((e) => {
          return e.joinConfig.channelId == this.last_at_vc_channel.id;
        })
      ) {
  ...
  cleanConnectionArr(filteredEle) {
    filteredEle.forEach((f) =>
      this.connections.splice(
        this.connections.findIndex((e) => {
  ...
  async sendInpModal(interaction) {
    const ytInpModal = new ModalBuilder().setCustomId("add_inp").setTitle("Add song or list into queue!");
  ...
  async send_control_panel(args) {
    ...
    const row1 = new ActionRowBuilder().addComponents(
      ...
      new ButtonBuilder().setCustomId("cache_list").setLabel("📂Cache list").setStyle(ButtonStyle.Primary)
    );
    ...
    try {
      this.send_info_embed(this.queue[this.nowplaying], "Nowplaying", () => {
        return new Promise(async (resolve, reject) => {
          try {
            this.control_panel = await this.last_at_channel.send({
              embeds: [output_embed],
              files: [file],
    ...
    try {
      if (validateURL(inp_url)) {
        ...
        if (is_LIVE) {
          ...
          time_str = (video_sec - (video_sec % 60)) / 60 + ":" + (video_sec % 60).toString().padStart(2, "0");
        }
      } else {
        var sendURL = inp_url;
        if (await this.is_GD_url(inp_url)) {
          ...
          var result = await parser(inp_url);
          //console.log(result);
        ...
        await new Promise((resolve, reject) => {
          ffprobe(sendURL, function (err, probeData) {
            if (err) {
              resolve();
            }
            video_sec = probeData.format.duration % 60;
            time_str = (probeData.format.duration - video_sec) / 60 + ":" + parseInt(video_sec).toString();
            resolve();
          });
        });
      }
      ...
      this.last_at_channel
        .send({ embeds: [output_embed] })
        .then((msg) => {
  ...
  async fetch_url_to_queue(interaction) {
    ...
    const row1 = new ActionRowBuilder().addComponents(
      new ButtonBuilder().setCustomId("ytpl_toomuch_but").setLabel("Add anyway").setStyle(ButtonStyle.Primary)
    );
    ...
    try {
      ...
      if (validateID(inp_url)) {
        ...
        (await this.is_local_url_avaliabe(inp_url)) &&
      ...
      interaction.channel.send("Something went wrong,bot might not joined channel before" + `\`\`\`${inp_url}\`\`\``);
    }
      ...
      if (isLIVE) {
        ...
        if (begin_t && duration <= Math.ceil(begin_t / 1000)) {
          ...
          if (YTCache) {
            ...
            var YTTempUrl = this.format_local_absolute_url(join(music_temp_dir, "YTTemp/"));
            var file_url = this.format_local_absolute_url(join(YTTempUrl, file_name + ".opus"));
            var fileUrlWithoutFormat = this.format_local_absolute_url(join(YTTempUrl, file_name));
            ...
            if (fileDead || !this.is_local_url_avaliabe(file_url)) {
              ...
              var ytDlpEventEmitter = ytDlpWrap
                .exec([url, "--recode-video", "opus", "--embed-thumbnail", "-f", "bestaudio", "-o", fileUrlWithoutFormat])
  ...
  async play_GD_url(url, begin_t, force_download) {
    ...
    var file_name = (await parser(url)).og.title.toString().replace(/\:|\/|\\|\||\"|\*|\<|\>|\?/g, "");
    var file_url = this.format_local_absolute_url(join(music_temp_dir, file_name));
    ...
    if (search_cache.length == 0 || force_download) {
      const page_req = get(("https://drive.google.com/uc?export=open&confirm=yTib&id=" + GD_ID).toString(), (page_response) => {
        const f_req = get(page_response.headers.location, (response) => {
          ...
          var download_f = createWriteStream(file_url);
  ...
  play_local_stream(res, begin_t) {
    ...
    try {
      ...
      this.playAudioResauce(this.wrapStreamToResauce(createReadStream(res), begin_t));
    } catch (error) {
      throw error;
    }
  }
  ...
  createYTDLPStream(url) {
    ...
    return ytDlpWrap.execStream([url, "-f", "bestaudio"], {}, this.YTDLPAbortController.signal).on("error", (e) => {
      console.log("YTDLPLiveErr");
    });
  }
    ...
    readdir(dir, (err, files) => {
      if (!err) {
        ...
        files.forEach((file) => {
          // Print file name
          //console.log(file);
          var file_full_path = join(dir, file);
          ((file_full_path) => {
            // Get file stat
            stat(file_full_path, (err, stats) => {
              if (!err) {
  ...
  cache_queue_io(file_full_path, is_add_to_pl = false) {
    ...
    stat(file_full_path, (err, stats) => {
      if (!err && stats.isFile() && this.is_file_type_avaliable(file_full_path)) {
        ...
        if (
          this.urlFilterByFileName(this.cached_file, file_full_path).length == 0 &&
  ...
  wrapStreamToResauce(stream, BT = false) {
    try {
      ...
      audio_resauce = createAudioResource(streamOpt, { inputType: StreamType.Arbitrary, silencePaddingFrames: 10 });
  ...
  async ytplTooMuchHandler(args) {
    ...
    if (this.ytpl_continuation) {
      ...
      while (go_flag) {
        try {
          playlist = await continueReq(this.ytpl_continuation);
  ...
  playingErrorHandling(resauce, errorInp, forceD = false) {
    ...
    try {
      ...
      if (errorInp == "Error: write after end") {
        ...
        console.log(
          "The error might cause memory leak, please restart the program or enable YTCache;\nif already done,please call support"
        );
